%function lineageMapperWitoutGUI(subDir,wellName)
    ...
    function Lineage_Mapper(subDir,wellName,trackAnswer)
    ...
    if ~isdeployed
        addpath([pwd filesep 'LineageMappersrc']);
        addpath([pwd filesep 'LineageMapperimgs']);
        addpath([pwd filesep 'LineageMapperdoc']);
    end
    ...
    segmented_images_path = strcat(subDir,'SegmentationOutput',filesep);
    tracked_images_path = strcat(segmented_images_path,'Tracking',filesep);
    mkdir(tracked_images_path);
    segmented_images_common_name = wellName;
    raw_images_common_name = wellName;
    tracked_images_common_name = strcat('Tracked_',wellName,'_');

    % Cost function params
    weight_overlap = str2num(trackAnswer{1});
    weight_centroids = str2num(trackAnswer{2});
    weight_size = str2num(trackAnswer{3});
    max_centroids_distance = str2num(trackAnswer{4});
    frames_to_track = 'All';

    % Mitotic params
    division_overlap_threshold = str2num(trackAnswer{5});
    daughter_size_similarity = str2num(trackAnswer{6});
    daughter_aspect_ratio_similarity = str2num(trackAnswer{7});
    circularity_threshold = str2num(trackAnswer{8});
    number_of_frames_check_circularity = str2num(trackAnswer{9});
    enable_cell_mitosis_flag = str2bool(trackAnswer{10});

    % Confidence Index params
    cell_life_threshold = str2num(trackAnswer{11});
    cell_apoptosis_delta_centroid_thres = str2num(trackAnswer{12});
    cell_density_ci_flag = str2num(trackAnswer{13});
    border_cell_ci_flag = str2num(trackAnswer{14});


    % Fusion params
    cell_size_threshold = str2num(trackAnswer{15});
    fusion_overlap_threshold = str2num(trackAnswer{16});
    enable_cell_fusion_flag = str2bool(trackAnswer{17});
...
%         filepath = tracked_images_path;
%         imgs = dir([filepath '*.tif']);
%         imgs = {imgs.name};
%         centroids = cell(numel(imgs),1);
%         for i = 1:numel(imgs)
%             L = imread([filepath imgs{i}]);
...
%         %centroids{i}(cell_id,:)
    ...
    %--------------------------------------------------------------------------------------------------
        ...
        function select_folder_segmented_images_Callback(varargin)
            ...
            if sdir ~= 0
                try
                    segmented_images_path = validate_filepath(sdir);
                catch err
                    if (strcmp(err.identifier,'validate_filepath:notFoundInPath')) || ...
                            (strcmp(err.identifier,'validate_filepath:argChk'))
                        errordlg('Invalid directory selected');
    ...
    %----------------------------------------------------------------------------
        ...
        function select_folder_tracked_images_Callback(varargin)
            ...
            if sdir ~= 0
                try
                    tracked_images_path = validate_filepath(sdir);
                catch err
                    if (strcmp(err.identifier,'validate_filepath:notFoundInPath')) || ...
                            (strcmp(err.identifier,'validate_filepath:argChk'))
                        errordlg('Invalid directory selected');
